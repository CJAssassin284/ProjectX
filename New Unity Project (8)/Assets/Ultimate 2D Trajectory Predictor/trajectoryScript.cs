...
public class trajectoryScript : MonoBehaviour {
	...
	public bool ballIsClicked2 = false;		//If the finger/cursor is pressing down in the "Ball Click Area" to activate the shot
	...
	void Update () {
        ...
        if(grab)
        {
            transform.position = claw.transform.position + offset;
        }

        if(isHeld)
        {
            transform.RotateAround(holder.transform.position, zAxis, Time.deltaTime * 100);
        }
        if(speed)
        {
            ballRB.velocity = Vector2.up * 10;
    ...
    {
        if(collision.CompareTag("Holder") && ballRB.isKinematic == false)
        {
            ...
            collision.GetComponent<HolderScript>().canGrab = false;
            if (lava.start == false)
            {
            //    gm.GetComponent<PowerUpSpawner>().enabled = true;
              //  gm.GetComponent<DestroySpawner>().enabled = true;

                // lava.start = true;
           //     lavaFill.gameObject.SetActive(true);
            }
        }
        ...
        if (collision.CompareTag("Claw"))
        {
            ...
            grab = true;
            collision.GetComponent<Claw>().goingUp = true;
        }

        if (collision.CompareTag("Speed"))
        {
            speed = true;
            GetComponent<Combo>().SpeedSpawn(collision.transform);
        }
    }
    private void OnTriggerStay2D(Collider2D collision)
    {
        if (collision.CompareTag("Holder") && ballIsClicked)
        {
            ballRB.isKinematic = true;
            ballRB.gravityScale = 0f;
        }
        else if(collision.CompareTag("Holder") && !ballIsClicked)
        {
            isHeld = true;
        }
    }
    ...
    {
        if (collision.CompareTag("Holder") && isHeld)
        {
            ballRB.isKinematic = false;
            isHeld = false;
            ballRB.gravityScale = grav;
            collision.GetComponent<HolderScript>().canDrop = false;
        }
    }

    public void ExitHolder()
    {
            ballRB.isKinematic = false;
            isHeld = false;
            ballRB.gravityScale = grav;
